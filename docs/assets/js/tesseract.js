...
;(function () {
  ...
  function initBg(el) {
    ...
    var FRAG_COMP = [
      ...
      '  float rawAlpha=max(max(col.r,col.g),col.b);',
      '  float alpha=smoothstep(.02,.08,rawAlpha);',
      '  float boost=mix(1.3,1.25,D);',
      '  col=mix(bg,col*boost,alpha);',
      '',
      '  // Particles — only where scene is pure background (rawAlpha near 0)',
      '  float bgMask=1.-smoothstep(.005,.015,rawAlpha);',
      '  vec2 gp=floor(gl_FragCoord.xy/2.);',
      '  float h=hash(gp);',
      '  if(D>.5&&h>.994){',
      '    float group=floor(mod(h*1000.,4.));',
      '    float phase=group*1.5708;',
      '    float speed=1.8+hash(gp+55.)*.8;',
      '    float raw=sin(T*speed+phase+hash(gp+33.)*6.28);',
      '    float pulse=raw*.5+.5;',
      '    pulse=pulse*pulse;',
      '    float maxB=.8+hash(gp+88.)*.2;',
      '    float twinkle=.08+pulse*maxB;',
      '    float brightness=smoothstep(.994,1.,h)*twinkle;',
      '    vec3 starCol=mix(vec3(.8,.85,1.),vec3(1.,.95,.8),hash(gp+99.));',
      '    col+=starCol*brightness*bgMask*1.3;',
      '  }',
      '  if(D<.5&&h>.993){',
      '    float group=floor(mod(h*1000.,4.));',
      '    float phase=group*1.5708;',
      '    float speed=1.5+hash(gp+55.)*.7;',
      '    float raw=sin(T*speed+phase+hash(gp+33.)*6.28);',
      '    float pulse=raw*.5+.5;',
      '    pulse=pulse*pulse;',
      '    float maxB=.7+hash(gp+88.)*.2;',
      '    float shimmer=.05+pulse*maxB;',
      '    float brightness=smoothstep(.993,1.,h)*shimmer;',
      '    vec3 dustCol=mix(vec3(.35,.2,.55),vec3(.5,.3,.7),hash(gp+77.));',
      '    col-=dustCol*brightness*bgMask*.7;',
      '  }',
      '',
      '  O=vec4(col,1);',
      '}'
