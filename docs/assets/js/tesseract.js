...
;(function () {
  ...
  function initBg(el) {
    ...
    var FRAG_COMP = [
      ...
      '  float h=hash(gp);',
      '  if(D>.5&&h>.997){',
      '    float twinkle=.6+.4*sin(T*(.8+h*3.)+h*100.);',
      ...
      '  }',
      '  if(D<.5&&h>.998){',
      '    float shimmer=.4+.3*sin(T*(1.2+h*2.)+h*80.);',
      '    float brightness=smoothstep(.998,1.,h)*shimmer;',
      '    vec3 dustCol=mix(vec3(.7,.6,.9),vec3(.85,.75,1.),hash(gp+77.));',
      '    col+=dustCol*brightness*(1.-alpha)*.25;',
      '  }',
      '',
      '  O=vec4(col,1);',
      '}'
