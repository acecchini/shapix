...
=== "NumPy"
    ...
    - `Shaped`: any dtype at runtime, with shape-only checking

    Notes:

    - `DT64` and `TD64` accept unit-qualified NumPy dtypes such as `datetime64[ns]`,
        `datetime64[D]`, `timedelta64[ms]`, and `timedelta64[s]`.
    - `Shaped` checks shape only at runtime. Its static alias is necessarily an
        approximation, not a true "any dtype ndarray" model.
...
=== "CuPy"
    ...
    It does **not** expose `BF16`, `F128`, `C256`, or the NumPy-only non-numeric
    aliases `V`, `Str`, `Bytes`, `Obj`, `DT64`, and `TD64`.
...
That makes it the right tool when the built-in alias set is close but not quite
what you need.

## When to use array types vs Like types

- Use array types such as `F32[N, C]` when the function truly requires a real
    backend array object.
- Use `Like` types such as `F32Like[N, C]` when callers may pass scalars, lists,
    tuples, or convertible foreign arrays that you immediately normalize
    yourself.
